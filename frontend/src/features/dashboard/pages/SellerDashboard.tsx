    ...
    <AppLayout>
      <div className="min-h-screen p-4 md:p-6 lg:p-8" style={{ background: 'var(--bg)', color: 'var(--text1)' }}>
        ...
        <section className="mt-6 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
          {stats.map(item => (
            <article key={item.label} className="rounded-xl p-4" style={{ background: 'var(--bg)', border: '1px solid var(--border-custom)' }}>
              <p className="text-xs uppercase tracking-wide" style={{ color: 'var(--text2)' }}>{item.label}</p>
              <p className="mt-2 text-xl font-bold">{item.value}</p>
              <p className="mt-1 text-xs" style={{ color: 'var(--text2)' }}>{item.note}</p>
            </article>
          ))}
        </section>
            ...
            <article className="rounded-2xl p-5" style={{ background: 'var(--bg)', border: '1px solid var(--border-custom)' }}>
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  ...
                  className="text-sm" style={{ color: 'var(--link)' }}
                >
                  View all listings
                </button>
              </div>
                ...
                {topListings.map(item => (
                  <div key={item.name} className="rounded-xl border p-4" style={{ background: 'var(--bg2)', borderColor: 'var(--border-custom)' }}>
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                      ...
                      <span className="text-xs rounded-full border px-2.5 py-1" style={{ background: 'var(--bg)', borderColor: 'var(--border-custom)', color: 'var(--text3)' }}>{item.status}</span>
                    </div>
                    <div className="mt-3 grid grid-cols-3 gap-2">
                      <div>
                        <p className="text-xs" style={{ color: 'var(--text2)' }}>Views</p>
                        <p className="text-sm font-semibold">{item.views}</p>
                      </div>
                      <div>
                        <p className="text-xs" style={{ color: 'var(--text2)' }}>Saves</p>
