      ...
      <div className="min-h-screen p-4 md:p-6 lg:p-8" style={{ background: 'var(--bg)', color: 'var(--text1)' }}>
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2">
                <span className="rounded-full px-3 py-1 text-xs font-medium" style={{ background: 'var(--bg2)', border: '1px solid var(--border-custom)', color: 'var(--text3)' }}>Logged in as {profileName}</span>
                <span className="rounded-full px-3 py-1 text-xs font-medium" style={{ background: 'var(--bg2)', border: '1px solid var(--border-custom)', color: 'var(--text3)' }}>Role: {user?.role ?? 'buyer'}</span>
                <span className="rounded-full px-3 py-1 text-xs font-medium" style={{ background: 'var(--bg2)', border: '1px solid var(--border-custom)', color: 'var(--text3)' }}>Dashboard: /buyer/dashboard</span>
              </div>
            </div>
        ...
        <section className="mt-6 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
          {summaryCards.map(card => (
            <article key={card.label} className="rounded-xl p-4" style={{ background: 'var(--bg)', border: '1px solid var(--border-custom)', color: 'var(--text1)' }}>
              <p className="text-xs uppercase tracking-wide" style={{ color: 'var(--text2)' }}>{card.label}</p>
              <p className="mt-2 text-xl font-bold">{card.value}</p>
              <p className="mt-1 text-xs" style={{ color: 'var(--text2)' }}>{card.note}</p>
            </article>
          ))}
        </section>
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-2">
                    {socialInfluence.map(item => (
                      <div key={item.label} className="rounded-lg border px-3 py-2" style={{ background: 'var(--bg2)', borderColor: 'var(--border-custom)' }}>
                        <p className="text-xs" style={{ color: 'var(--text2)' }}>{item.label}</p>
                  ...
                  <div key={order.id} className={`rounded-xl border p-4 ${soft}`}>
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                      <div>
                        ...
                        <p className="text-sm" style={{ color: 'var(--text2)' }}>{order.id} • Sold by {order.seller}</p>
                      </div>
                      <p className="font-bold">{order.amount}</p>
                    </div>
                    <div className="mt-2 flex items-center justify-between">
                      <span className="text-xs px-2.5 py-1 rounded-full border" style={{ background: 'var(--bg2)', borderColor: 'var(--border-custom)', color: 'var(--text3)' }}>{order.status}</span>
                      <span className="text-xs" style={{ color: 'var(--text2)' }}>{order.eta}</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <article className={`rounded-2xl p-5 ${surface}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {recommended.map(item => (
                    <div key={item.name} className="rounded-xl border p-4" style={{ background: 'var(--bg2)', borderColor: 'var(--border-custom)' }}>
                      <div className="h-24 rounded-lg mb-3" style={{ background: 'var(--bg)' }} />
