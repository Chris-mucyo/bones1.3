...
function SuggestedCard({ listing, index }: { listing: Listing; index: number }) {
  ...
  return (
    <div
      ...
      className="flex gap-3 cursor-pointer group rounded-xl p-2.5 transition-colors hover:bg-black/[0.04]"
    >
      ...
      <div className="flex-1 min-w-0 py-0.5">
        <p className="text-[12.5px] font-semibold leading-snug line-clamp-2 mb-1.5 transition-colors" style={{ color: 'var(--text1)' }}>
          {listing.title}
        </p>
                ...
                {loadingSide ? <SkeletonSidebar /> : (
                  <div className="flex flex-col gap-1">
                    {related.map((l) => {
                      ...
                      return <SuggestedCard key={l.id} listing={l} index={relIdx} isDark={theme === 'dark'} />;
                    })}
                    {!related.length && (
